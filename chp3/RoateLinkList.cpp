#include"LinkList.h"
#include<iostream>
void RotateLinkListWithPosition(LinkListNode** iHead, int iPos)
{
    LinkListNode* lNode = *iHead;
    int i = 1;
    while (i < iPos && lNode != nullptr)
    {
        lNode = lNode->next;
        i++;
    }
    if(lNode == nullptr)
        return ; // iPos is less then link list size. we can do nothing. 
    LinkListNode * lCurrent = lNode->next;
    lNode->next = nullptr;
    lNode = lCurrent;
    while(lNode->next)
    {
...
void case_RotateLinkListWithPosition_acceptance()
{
    ...
    // iPos ==4, expect to be 50->60->10->20->30->40
    RotateLinkListWithPosition(&lHead ,4 );
